...
void GameMain::update(float time){
    ...
    for(int i = 0;i < 5;i ++){
        if(p1.x <= 100 && (p1.x + 480) >= 100){
			...
			GameObjStar1* obj1 = (GameObjStar1 *)(map->stars3)->objectAtIndex(i);
			if(obj1->get_visable() && isCollion(ccp(100,hero->getPosition().y + 62.5),ccp(p1.x + 86 + 96 * i,180),40,35,18.25,17.75)){
				obj1->set_visable(false);
			...
			GameObjStar1* obj1 = (GameObjStar1 *)(map->stars4)->objectAtIndex(i);
			if(obj1->get_visable() && isCollion(ccp(100,hero->getPosition().y + 62.5),ccp(p2.x + 86 + 96 * i,180),40,35,18.25,17.75)){
				obj1->set_visable(false);
				gamemark->addnumber(-20);
			}
        }
