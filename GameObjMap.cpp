...
void GameObjMap::bg1change(){
    ...
    for(int i = 0;i < 5;i ++){
		...
		((GameObjStar *)stars3->objectAtIndex(i))->set_visable(true);
    }
}
void GameObjMap::bg2change(){
    ...
    for(int i = 0;i < 5;i ++){
		...
		((GameObjStar *)stars4->objectAtIndex(i))->set_visable(true);
    }
}
void GameObjMap::onEnter(){
   ...
   // stars2 = CCArray::create(5);
	...
	stars3=new CCArray(5);
	stars4=new CCArray(5);
    for(int i = 0;i < 5;i ++){
		//将星星加入到场景中
		...
		GameObjStar1* obj1 = new GameObjStar1();
		obj1->setPosition(ccp(172 + 192 * i,200));
		stars3->addObject(obj1);
		bg1->addChild(obj1,3);

		obj1 = new GameObjStar1();
		obj1->setPosition(ccp(172 + 192 * i,200));
		stars4->addObject(obj1);
		bg2->addChild(obj1,3);
    }
    stars1->retain();
    stars2->retain();
